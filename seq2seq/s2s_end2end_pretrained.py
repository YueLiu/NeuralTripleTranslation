...
from utils.seq2seq.learning_core_20180331 import Seq2seqCore
...
def s2s_end2end(filename):
    ...
    for epoch_number in range(1, 300):
        print(epoch_number)
        s2s_core.load("models/20180331/pso/%05d.tfmodel"%epoch_number)
        predict_result = s2s_core.predict(input_dict)
        predict_list = predict_result.sample_id[:, 0:3].tolist()
        f_w = open("data/output/pso_attention_pretrained_%03d.txt"%epoch_number, "w")
